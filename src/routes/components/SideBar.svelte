	...
	<nav class="flex flex-col">
		...
		<dialog id="c">
			<form enctype="multipart/form-data" on:submit|preventDefault>
				...
				<input type="text" id="modalName" name="modalName" bind:this={companyName} required /><br
				/><br />

				<button
					type="submit"
					on:click={() => {
						data1.update((prev) => [...prev, { companyName: companyName.value, models: [] }]);
						c.close();
					}}>Submit</button
				>
				<button
		...
		<img
			...
			class="block w-[4rem] self-center pt-4 hover:cursor-pointer"
			src={addButton}
			alt=""
		/>
	</nav>
